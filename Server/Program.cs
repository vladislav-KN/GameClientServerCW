...
namespace Server
{
    ...
    where pl.Email = @eml OR usr.[Login] = @log;";
                        ...
                        cmd.Parameters.AddWithValue("@log", RegisterData.Login);
                        cmd.ExecuteNonQuery();
                        strSQL = @" INSERT INTO Player (Email, NickName) values(@eml, @nic)
        SELECT SCOPE_IDENTITY() AS [ID];";
                        ...
                        cmd.Parameters.AddWithValue("@log", RegisterData.Login);
                        cmd.ExecuteNonQuery();
                        Player newPlayer = new Player(RegisterData.Login, RegisterData.Password, RegisterData.Email, ID, RegisterData.NickName, 0, new DateTime(0), new List<GameClass.Objects.Item>(), 0, 0, new List<Game>());
